...
/**
 * Provides a graphical representation of the game world.
 */
public class GameView {

  /**
   * Displays the game world map and saves it to an image file.
   *
   * @param world the game world to display
   * @param imageOutputPath the path to save the generated image
  ...
  public void displayMap(Igameworld world, String imageOutputPath) throws IOException {
    ...
    BufferedImage image = new BufferedImage(world.getColumns() * cellSize + 2 * padding,
                                           world.getRows() * cellSize 
                                           + 2 * padding, BufferedImage.TYPE_INT_ARGB);
    Graphics g = image.getGraphics();
    g.setColor(Color.WHITE);
    g.fillRect(0, 0, world.getColumns() * cellSize 
        + 2 * padding, world.getRows() * cellSize + 2 * padding);
    ...
    for (Iroom room : world.getRooms()) {
      ...
      for (Iitem item : room.getItems()) {
        g.drawString(item.getName(), x + width 
            - g.getFontMetrics().stringWidth(item.getName()) - 2, y + itemOffsetY);
        itemOffsetY += 15;
      }
      g.setColor(Color.BLACK); 
    }
    ...
    try {
      ...
      e.printStackTrace();
      throw new IOException("Failed to save the image to " + imageOutputPath, e);
    }
  }
}
